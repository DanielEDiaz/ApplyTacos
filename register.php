<?php
	...
	if(isset($_POST['signup'])){
		...
		if(!isset($_SESSION['captcha'])){
			...
			if (!$resp->isSuccess()){
		  		$_SESSION['error'] = 'Por favor conteste el captcha correctamente.';
			...
			if($row['numrows'] > 0){
				...
				try{
				    ...
				    try {
				        ...
				        $_SESSION['success'] = 'Cuenta creada. Revisa tu correo para activarla.';
				        header('location: signup.php');

				    } 
				    catch (Exception $e) {
				        $_SESSION['error'] = 'El mensaje no pudo ser enviado: '.$mail->ErrorInfo;
				        header('location: signup.php');
				    }
		...
		$_SESSION['error'] = 'Primero llene el formulario de registro';
		header('location: signup.php');
	}
